...
export class FallbackGenerator implements Fixer {
  ...
  async fix(projectPath: string, issue: Issue): Promise<Fix> {
    ...
    try {
      ...
      // Build description
      let description = `Generated WebGL fallback for ${webglLib ?? 'generic'} using template`;
      if (integration.note) {
  ...
  private async renderTemplate(library: string | null, context: TemplateContext): Promise<string | null> {
    try {
      ...
      this.log.warn(`Failed to render template (${library ?? 'generic'}): ${err instanceof Error ? err.message : String(err)}`);
      return null;
    }
  }
...
`
  ...
  private async integrateIntoEntryPoint(projectPath: string, uiFramework: UIFramework): Promise<{ file?: string; note?: string }> {
