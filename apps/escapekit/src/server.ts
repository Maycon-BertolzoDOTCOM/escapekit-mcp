...
import { AnalyzeSandboxCodeInputSchema, safeParseConfig } from './schemas/index.js';
...
server.registerTool({
  ...
  execute: async (params: MCPParams) => {
    ...
    const validation = safeParseConfig(AnalyzeSandboxCodeInputSchema, params);
    ...
    const { code, sandboxType, options } = validation.data;

    return await analyzeCode(
      code,
      sandboxType === 'auto' ? undefined : sandboxType,
