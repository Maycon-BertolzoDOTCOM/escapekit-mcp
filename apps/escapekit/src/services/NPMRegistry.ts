...
export class NPMRegistry {
  ...
  /**
   * Retry operation using the shared withRetry utility.
   * Respects PackageNotFoundError as non-retryable.
   */
  private async retryOperation<T>(
    ...
    return withRetry(operation, {
      ...
      baseDelay: this.config.initialRetryDelay,
      maxDelay: 32000,
      isRetryable: (error) => {
        // Don't retry definitive 404s
        if (error instanceof PackageNotFoundError) return false;
        return true;
      },
      onRetry: (attempt, delay, error) => {
        this.logger.warn(
          `${operationName} failed for ${packageName} (attempt ${attempt}), retrying in ${delay}ms`,
          { error: error.message },
        );
      },
    });
