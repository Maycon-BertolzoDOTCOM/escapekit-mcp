...
import { TransformationType, } from '../models/transformation.js';
/**
...
export class ImportReplacer {
     ...
     * ```
     */
    replaceImports(sourceCode, resolutions) {
        let ast;
        try {
            ast = this.astTransformer.parseWithRecast(sourceCode);
        }
        catch (err) {
            throw new TransformationError(`Failed to parse source code: ${err.message}`, 'parseWithRecast', { sourceCode: sourceCode.slice(0, 200) });
        }
        ...
        try {
            ...
            throw new TransformationError(`Failed to generate transformed code: ${err.message}`, 'generate', { sourceCode: sourceCode.slice(0, 200) });
        }
     ...
     * ```
     */
    generateDiff(original, transformed) {
     ...
     */
    buildUnifiedDiff(originalLines, transformedLines, originalLabel, transformedLabel) {
     ...
     */
    computeHunks(originalLines, transformedLines) {
     ...
     */
    computeEdits(originalLines, transformedLines) {
    ...
    countChangedLines(diff) {
        if (!diff)
            return 0;
        return diff.split('\n').filter(line => line.startsWith('+') || line.startsWith('-')).filter(line => !line.startsWith('+++') && !line.startsWith('---')).length;
    }
}
//# sourceMappingURL=ImportReplacer.js.map