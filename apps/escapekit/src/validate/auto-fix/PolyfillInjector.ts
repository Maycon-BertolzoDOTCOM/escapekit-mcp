...
export class PolyfillInjector implements Fixer {
  ...
  private async integrateVite(
    projectPath: string
  ): Promise<{ file: string | undefined; note?: string }> {
    ...
    for (const entry of entryPoints) {
      ...
      if (await this.hasFile(projectPath, entry)) {
        ...
        if (
          !content.includes("import './polyfills'") &&
          !content.includes('import "./polyfills"')
        ) {
  ...
  private async integrateWebpack(projectPath: string): Promise<{ file: string; note?: string }> {
    ...
    for (const configFile of configFiles) {
      ...
      if (await this.hasFile(projectPath, configFile)) {
        ...
        // Transform entry: handle string first, then array (only one should match)
        if (/entry:\s*['"]/.test(content)) {
          content = content.replace(
            /entry:\s*['"]([^'"]+)['"]/g,
            "entry: ['$1', './src/polyfills']"
          );
        } else {
          content = content.replace(/entry:\s*\[([^\]]+)\]/g, "entry: [$1, './src/polyfills']");
        }
  ...
  private async integrateNextjs(projectPath: string): Promise<{ file: string; note?: string }> {
    const result: { file: string; note?: string } = { file: '', note: undefined };
