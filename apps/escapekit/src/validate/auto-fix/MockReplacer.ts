...
export class MockReplacer implements Fixer {
  ...
  // File operation methods — can be overridden in tests
  ...
  private async applyReplacement(
    ...
    const filePath = join(projectPath, issue.file);
    let content = await this.readFile(filePath, 'utf-8');
    let changed = false;
    ...
    if (changed) {
      await this.writeFile(filePath, content, 'utf-8');
  ...
  private async updatePackageJson(
    ...
    try {
      const raw = await this.readFile(pkgPath, 'utf-8');
