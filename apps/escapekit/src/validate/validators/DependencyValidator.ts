...
export class DependencyValidator {
    ...
    try {
      ...
      if (audit.vulnerabilities) {
        for (const [name, vuln] of Object.entries(
          audit.vulnerabilities as Record<string, VulnerabilityInfo>
        )) {
          vulnerabilities.push({
            ...
            title:
              typeof vuln.via?.[0] === 'string'
                ? vuln.via[0]
                : vuln.via?.[0]?.title || 'Unknown vulnerability',
            url: vuln.via?.[0]?.url,
      ...
      this.log.warn(
        `Failed to parse npm audit output: ${err instanceof Error ? err.message : String(err)}`
      );
      return [];
    }
  }
  ...
  private async checkOutdated(projectPath: string): Promise<OutdatedPackage[]> {
    ...
    try {
      ...
      this.log.warn(
        `Failed to parse npm outdated output: ${err instanceof Error ? err.message : String(err)}`
      );
      return [];
    }
  }
