...
export class SlopsquatDetector {
   ...
   */
  async analyze(packageName: string, metadata?: PackageMetadata, now: Date = new Date()): Promise<Issue | null> {
    ...
    if (metadata?.publishDate) {
      const daysSincePublish = (now.getTime() - metadata.publishDate.getTime()) / (1000 * 60 * 60 * 24);
