...
export class SecurityValidator {
  ...
  async validate(
    ...
    if (!maintained)
      warnings.push(
        `Package "${packageName}" has not been updated in >${this.options.maxStalenessMonths} months`
      );
  ...
  checkLastUpdate(lastUpdate?: string): boolean {
    ...
    try {
      ...
      if (isNaN(updateDate.getTime())) return true; // Invalid date - assume maintained
