...
export async function generateEscapeKit(
  ...
  try {
    ...
    if (!dryRun) {
      ...
      // Generate escape.json if enabled
      if (options.generateEscapeJson !== false && !dryRun) {
        try {
          ...
          await writer.writeToFile(escapeJson, join(projectPath, 'escape.json'));
          filesCreated.push('escape.json');
        } catch (err) {
          log.warn(`Failed to generate escape.json: ${(err as Error).message}`);
        }
      }
